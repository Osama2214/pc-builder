...
- **Shared infrastructure:** a single `apiRequest()` wrapper, custom-styled replacements for native `alert()`/`confirm()`/`<select>`/file inputs (matching the site's dark theme instead of default browser chrome), a shared cart drawer, and a shared navbar with a dynamically populated categories menu.

## 3. Not Currently Planned

- AI assistant / natural-language build recommendations.
...
- Production deployment (currently runs locally: Laravel's built-in server for the backend, PHP's built-in static server for the frontend).
